...
path_to_cleaned_files = r"C:\Users\krkc6\PycharmProjects\jobdescription\1_Collecting_Data\jds_skills"
...
def mapping(jd):
    ...
    for i in range(len(arr)):
        #print(arr[i])
        if arr[i].__contains__("skills") or arr[i].__contains__("requirements"):
            lis = [arr[i-5:i+5]]
            # for j in lis:
...
for i, a in enumerate(df.iterrows()):
    ...
    with open(r"C:\Users\krkc6\PycharmProjects\jobdescription\1_Collecting_Data\jds_skills\skills_csv.csv", mode='a', encoding='utf-8') as employee_file:
